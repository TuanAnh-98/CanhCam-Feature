...
namespace CanhCam.Web.UI
{
    ...
    public class Avatar : WebControl
    {
        ...
        private string GetAvatarMarkup()
        {
            ...
            {
                return "<img  alt='" + LinkTitle + "' src='" + GetInternalAvatarUrl() + "' class='avatar" + (CssClass.Length > 0 ? " " + CssClass : string.Empty) + "' />";
            }
            //}


            return "<a rel='nofollow' href='" + GetProfileUrl() + "' class='avatar" + (CssClass.Length > 0 ? " " + CssClass : string.Empty) + "'><img  alt='" + GetAltText() + "' src='" + GetInternalAvatarUrl() + "' /></a>";
        ...
        private string GetProfileUrl()
        {
            ...
            if (userId > -1)
            {
                return SiteRoot + "/Secure/ProfileView.aspx?userid=" + userId.ToInvariantString();
            }
        ...
        protected void RenderGravater(HtmlTextWriter output)
        {
            ...
            {
                imageUrl += "&default=" + HttpUtility.UrlEncode(WebUtils.ResolveServerUrl(defaultInternalAvatar));
            }
