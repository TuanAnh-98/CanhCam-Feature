...
$(document).ready(function() {
    ...
    if ($('#edit-submit').length) {
        $('#edit-submit').click(function() {
            ...
            //if (address != '') {   
            gmap_location_lookup(address,distance, region,lat,long);  
            //} else {
    ...
    function delaylocationload() {
        ...
        //    gmap_location_lookup('', $('#distance').val(), '', data.latitude, data.longitude);
        //});
                    ...
                    success: function(msg) {
                        ...
                        if (results.length > 0) {
                            ...
                            jQuery.each(results, function(k, v) {
                                ...
                                distancenode="0"; 
                                if (v.getAttribute("distance") != "") {
                                    distancenode = v.getAttribute("distance");
                                } 
                                 
                                if (number > 9) {
                                    $("<li id='l_" + marker['__gm_id'] + "' class='clinic_list double-digit' />")
                                        .html('<p class="name">' + v.getAttribute("name") + '</p><p class="address">' + v.getAttribute("address") + '</p><p class="distance"><span class="text-main">' + distancenode + '</span><a class="text-blue" href="#">Xem bản đồ</a></p>')
                                        .click(function() {
...
function info_window_content(v) {
    ...
    //info_window_string += "<a href='javascript:direction(\"" + v.getAttribute("address") + "\"," + v.getAttribute("lat") + "," + v.getAttribute("lng") + ");'>" + lang[currCulture].direction + "</a>";
