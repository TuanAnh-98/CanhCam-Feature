...
<div class="admin-content col-md-12">
    <portal:HeadingPanel ID="heading" runat="server">
        <asp:Button SkinID="UpdateButton" ID="btnExport" Text="Export excel"
            runat="server" />
        <asp:Button SkinID="UpdateButton" ID="btnExportCategory" Text="Export Category excel"
            runat="server" />
    ...
    <div class="workplace">
        <telerik:RadGrid ID="grid" SkinID="radGridSkin" runat="server">
            <MasterTableView DataKeyNames="ProductId,ZoneId,DisplayOrder,ViewCount,Code,Price,OldPrice,IsPublished">
                <Columns>
                    ...
                    <telerik:GridTemplateColumn HeaderText="<%$Resources:ProductResources, ProductPictureHeading %>">
                        <ItemTemplate>
                            <portal:MediaElement ID="ml" runat="server" Title=" " Width="90" FileUrl='<%# ProductHelper.GetImageFilePath(siteSettings.SiteId, Convert.ToInt32(Eval("ProductId")), Eval("ImageFile").ToString(), Eval("ThumbnailFile").ToString()) %>' />
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
                    ...
                    <telerik:GridTemplateColumn HeaderStyle-Width="100" HeaderText="<%$Resources:ProductResources, IsPublishedLabel %>"
                        AllowFiltering="false">
                        <ItemTemplate>
                            <asp:CheckBox ID="cbPublished" Checked='<%# Convert.ToBoolean(Eval("IsPublished").ToString()) %>' runat="server" />
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
                    <telerik:GridTemplateColumn>
                        <ItemTemplate>
                            ...
                            <asp:HyperLink CssClass="cp-link" ID="ViewProduct" runat="server" Text="<%$Resources:NewsResources, NewsEditPreviewLabel %>"
                                NavigateUrl='<%# ProductHelper.FormatProductUrl(Eval("Url").ToString(), Convert.ToInt32(Eval("ProductId")), Convert.ToInt32(Eval("ZoneId"))) %>'>
                            </asp:HyperLink>
                        </ItemTemplate>
                    </telerik:GridTemplateColumn>
