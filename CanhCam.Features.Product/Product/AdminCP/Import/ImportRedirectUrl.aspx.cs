...
{
    public partial class ImportRedirectUrlPage : CmsNonBasePage
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(ImportRedirectUrlPage));
                ...
                for (i = titleRowNumber + 1; i <= worksheet.LastRowNum; i++)
                {
                    ...
                    if (dataRow != null)
                    {
                        ...
                        RedirectInfo redirect = new RedirectInfo
                        {
                            ...
                            OldUrl = GetPathFromUrl(oldUrl),
                            NewUrl = GetPathFromUrl(newUrl)
                        };
                        if (redirect.Save())
                        {
                            builder.Append($"<p>Old Url : {redirect.OldUrl}   =>  New Url  : {redirect.NewUrl}</p>");
                            count++;
                        }
                    }
                }
